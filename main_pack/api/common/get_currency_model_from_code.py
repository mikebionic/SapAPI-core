...
def get_currency_model_from_code(
	...
	session = None,
	Currency_dbModels = None,
):

	if not Currency_dbModels:
		Currency_dbModels = Currency.query.filter_by(GCRecord = None).all()

	if session:
		if "currency_code" in session:
			currency_code = session["currency_code"] if session["currency_code"] else currency_code

	currency = [currency for currency in Currency_dbModels if currency.CurrencyCode == currency_code.upper()]
