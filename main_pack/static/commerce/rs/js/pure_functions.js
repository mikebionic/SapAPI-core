...
function set_local_data_by_name(
	data_name,
	data_payload,
	stringify_json = true,
) {
...
function makeCartRequests(payload_data, url, type, responseForm, listName) {
	$.ajax({
...
$(document).ready(function () {
	...
	var local_cart_data = get_local_data_by_name('cart')
	var local_currency_data = get_local_data_by_name(
		'currency_code',
		(parse_json = false),
	)

	if (
		local_cart_data == undefined ||
		local_currency_data != session_currency_code
	) {
		local_cart_data = {}
		set_local_data_by_name('cart', local_cart_data)
	}
