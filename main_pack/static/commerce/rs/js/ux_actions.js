...
function totalPriceCheckout(ownerId) {
	...
	var productTotalPrice = parseFloat(
	...
	if (payment_method) {
		if (payment_method > 0) {
			if (payment_method == 2) {
				...
				if (
					session_currency_code !== 'TMT' &&
					online_payment_type == 'halkbank'
				) {
					$(
						'.online_payment_types .foreign_affairs_bank input',
					)[0].checked = true
					errorToaster(
						(message = `Can't checkout it it with currency ${session_currency_code}`),
					)
				}
			} else {
...
$('body').delegate('.checkoutCartBtn', 'click', function () {
		...
		if (payment_method && payment_method > 0) {
			...
			if (payment_method == 2) {
				...
				if (
					...
					errorToaster(
						(message = `Can't checkout it with currency ${session_currency_code}`),
					)
				}
			}
		}
	...
	if (PmId == 2) {
		...
		gen_Reg_no_and_open_payment(
			gen_reg_no_data,
			`${url_prefix}/ui-gen-reg-no/`,
			'POST',
		)
	} else {
