...
from main_pack.models_test.base.models import Slider,Sl_image
from main_pack.commerce_test.admin.utils import addSliderDict,addSliderImageDict
	...
	try:
		if request.method == 'POST':
			...
			if (sliderId == '' or sliderId == None):
				...
				db_test.session.add(newSlider)
				db_test.session.commit()
				response = jsonify({
					'sliderId':newSlider.SlId,
					'status':'created',
					'responseText':gettext('Slider')+' '+gettext('successfully saved'),
					'htmlData': render_template('commerce_test/admin/sliderAppend.html',slider=newSlider)
					})
			else:
				print('updating')
