...
from main_pack.commerce_test.admin.utils import addCategoryDict,addEditCategoryDict
	...
	try:
		if request.method == 'POST':
			...
			if (editCategoryId == '' or editCategoryId == None):
				...
				db_test.session.add(newCategory)
				...
				response = jsonify({
					...
					'htmlData':render_template('commerce_test/admin/appendCategory.html',child_status=child_status,category=newCategory)
					})
			else:
		...
		if request.method == 'POST':
			...
			if (categoryId == '' or categoryId == None):
				...
				response = jsonify({
					...
					'status':'created',
					'responseText':gettext('Category')+' '+gettext('successfully saved'),
					'htmlData': render_template('commerce_test/admin/categoryAppend.html',category=newCategory)
					})
			else:
				print('updating')
				thisCategory = Res_category.query.get(categoryId)
